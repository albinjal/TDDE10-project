...
/**
 * The level class is used as an interface for levels of the game. It holds
 * information about the initial conditions of the level such as distribution of
 * enemies and powerups. It also handles the logic of loading a new level where
 * it generates the correct amount of enemies and powerup which is then inserted
 * into the playmodel.
 * 
 * @author Albin
 * @version 1.0
...
public class Level {
	...
	private static ArrayList<Powerup> loadPowerUp(double dificulty, Rectangle2D outline, Powerups type,
			double multiplier) {
		ArrayList<Powerup> list = new ArrayList<Powerup>();
	...
	private static ArrayList<Enemy> loadEnemy(double dificulty, Rectangle2D outline, Enemies type, double multiplier,
			GameObject follow) {
	...
	private static MyPoint generatePos(Rectangle2D outline) {
		...
		switch (side) {
		case 0:
			return new MyPoint(0, generateDouble() * outline.getMaxY());
		case 1:
			return new MyPoint(generateDouble() * outline.getMaxX(), 0);
		case 2:
			return new MyPoint(outline.getMaxX(), generateDouble() * outline.getMaxY());
		case 3:
			return new MyPoint(generateDouble() * outline.getMaxX(), outline.getMaxY());
		default:
			return null;
		}
	}
